...
def get_stock_close_price(stock_code: str, target_date: datetime.date):
    """抓取指定日期前最近一個交易日的收盤價"""
    ...
    start_date = target_date - datetime.timedelta(days=10)  # 取前10天確保含交易日
    ...
    for ticker in tickers:
        ...
        if not df.empty and "Close" in df:
            ...
            return float(last_close), actual_date

    return None, "找不到該股票的歷史股價資料"


def calculate_hexagram(
    stock_code: str, date_obj: datetime.date, prev_close: float
):
...
st.caption("結合 yfinance 自動抓價與梅花易數，預測股價走勢")
...
if st.button("🔮 自動抓取股價並起卦預測", use_container_width=True):
    if not stock_input:
        ...
        # 1. 自動抓取基準日期的前一交易日收盤價
        with st.spinner("正在連線 Yahoo Finance 抓取歷史股價..."):
            prev_close, price_info = get_stock_close_price(
                stock_input, base_date
            )

        if prev_close is None:
            st.error(f"❌ 股價抓取失敗：{price_info}")
        else:
            st.success(f"📈 成功抓取基準股價（日期：{price_info}）：**{prev_close:.2f} 元**")
